shared class TmxMapInfo {
  ...
  TmxMapInfo(const Json::Value@ j) {
    ...
    this._TagList = array<int>(j["TagList"].Length);
    for (uint i = 0; i < j["TagList"].Length; i++) {
      this._TagList[i] = int(j["TagList"][i]);
    }
  }
  ...
  Json::Value@ ToJson() {
    ...
    j["Name"] = _Name;
    j["Tags"] = _Tags.ToJson();
    Json::Value@ _tmp_TagList = Json::Array();
    for (uint i = 0; i < _TagList.Length; i++) {
      auto v = _TagList[i];
      _tmp_TagList.Add(Json::Value(v));
    }
    j["TagList"] = _tmp_TagList;
    return j;
  }
  ...
  /* Methods // Mixin: Op Eq */
  bool opEquals(const TmxMapInfo@ &in other) {
    if (other is null) {
...
namespace _TmxMapInfo {
  ...
  shared TmxMapInfo@ ReadFromBuffer(MemoryBuffer@ buf) {
