shared class DictOfCompRoundMatch_WriteLog {
  ...
  _DictOfCompRoundMatch_WriteLog::KvPair@ GetItem(const string &in key) const {
    return _DictOfCompRoundMatch_WriteLog::KvPair(key, Get(key));
  }
  
  array<_DictOfCompRoundMatch_WriteLog::KvPair@>@ GetItems() const {
    array<_DictOfCompRoundMatch_WriteLog::KvPair@> ret = array<_DictOfCompRoundMatch_WriteLog::KvPair@>(GetSize());
  ...
  CompRoundMatch@ opIndex(const string &in key) {
    return Get(key);
  }
  ...
  private void InitLog(const string &in logDir, const string &in logFile) {
    ...
    trace('DictOfCompRoundMatch_WriteLog dir: ' + logDir + ' | logFile: ' + logFile);
    if (logDir.Length == 0) {
  ...
  private void LoadWriteLogFromDisk() {
    if (IO::FileExists(_logPath)) {
      ...
      while (!f.EOF()) {
        line = f.ReadLine();
        if (line.Length > 0) {
          auto kv = _DictOfCompRoundMatch_WriteLog::_KvPair::FromRowString(line);
          @_d[kv.key] = kv.val;
        }
      }
      f.Close();
      trace('\\$a4fDictOfCompRoundMatch_WriteLog\\$777 loaded \\$a4f' + GetSize() + '\\$777 entries from log file: \\$a4f' + _logPath + '\\$777 in \\$a4f' + (Time::Now - start) + ' ms\\$777.');
    } else {
  ...
  private void WriteOnSet(const string &in key, CompRoundMatch@ value) {
    _DictOfCompRoundMatch_WriteLog::KvPair@ p = _DictOfCompRoundMatch_WriteLog::KvPair(key, value);
...
namespace _DictOfCompRoundMatch_WriteLog {
  ...
  shared class KvPair {
    ...
    KvPair(const string &in key, CompRoundMatch@ val) {
      this._key = key;
      @this._val = val;
    }
